...
//! durable buffering as `nea_weather`, but each window is written to a
//! Hugging Face storage bucket (`hf://buckets/{namespace}/{bucket}`) via
//! `HfBucketSink` instead of committed to a dataset repo.
//!
//! Each pipeline's stack is `Tier(JsonlStore) → HfBucketSink`; graceful
//! shutdown preserves the active UTC window in its fsynced JSONL segment.
//! Closed windows land at deterministic Hive-style paths, uploaded through
//! Xet with chunk deduplication, then registered with one sans-IO
//! `BatchAction`. Buckets have no commit queue, so there is no `CommitGate`
//! to share.
//!
//! ```bash
...
use meathook::{HfBucketSink, Meathook, Pipeline, ShutdownPolicy};
...
impl SinkCtx {
    ...
    fn durable<R>(
        ...
        ctx: &Ctx,
        pipeline: &str,
    ) -> Result<common::DurableStack<R, HfBucketSink<R>>, meathook::HfBucketSinkError>
    where
        R: serde::Serialize + serde::de::DeserializeOwned + Send + 'static,
    {
        ...
        Ok(ctx.durable(pipeline, terminal))
    }
}
